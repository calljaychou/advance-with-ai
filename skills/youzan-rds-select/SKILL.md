...
使用此 skill 时，通过 RDS 查询配置和 `scripts/youzan_rds_select.sh` 调用有赞 RDS 查询接口，并基于模板中的 `resultValidate` 回答开发者问题。
...
- 当用户提出新增、补充或维护 config 时，只编辑 `references/rds-query-config.json`，不要执行 RDS 查询。
...
当用户提出“增加 config”“新增项目配置”“补充查询方法”“维护 RDS 配置”等需求时，按以下流程处理：

1. 先读取 `references/rds-query-config.json`，了解现有项目配置。
2. 如果用户没有提供完整配置，直接给出下面的 JSON 模板，让用户按模板回填。
3. 用户回填后，校验 JSON 字段完整性和 SQL 只读性。
4. 若 `dbName` 或 `keyword` 已存在，默认把新的 `queryMethods` 合并到已存在项目中。
5. 若项目不存在，直接追加为新的项目配置。
6. 保存后运行 JSON 格式校验，并告知用户保存结果。

配置模板：

```json
{
  "keyword": "项目中文名/项目英文名/常用简称",
  "dbName": "youzan-xxx",
  "bizInsIdMap": {
    "prod": "正式环境 bizInsId",
    "dev": "测试环境 bizInsId"
  ...
  "queryMethods": [
    {
      "method": "查询业务问题描述，例如：查询订单是否推送成功，若失败失败原因是什么",
      "resultValidate": "结果判断规则，例如：获取到的rows有数据或数量size大于1",
      "statement": "SELECT * FROM table_name WHERE biz_value='{业务单号}'"
    }
  ]
}
```
...
- 必填字段：`keyword`、`dbName`、`bizInsIdMap`、`appId`、`queryMethods`。
- `bizInsIdMap` 至少包含用户提供的环境，例如 `prod` 或 `dev`。
- `queryMethods` 至少包含一个查询方法。
- 每个查询方法必须包含 `method`、`resultValidate`、`statement`。
- `statement` 必须是 `SELECT` 查询，禁止保存 `INSERT`、`UPDATE`、`DELETE`、`DDL` 等变更语句。
- `statement` 中的动态值使用 `{变量名}` 占位，例如 `{订单号}`、`{退款单号}`。
- 若同一项目下出现完全相同的 `method`，先询问用户是覆盖旧方法还是保留两个方法；其他信息完整时不再二次确认，直接保存。

## 匹配流程

1. 读取 `references/rds-query-config.json`，用用户项目关键词匹配 `keyword` 字段，获取 `dbName`、`bizInsIdMap`、`appId` 和 `queryMethods`。
2. 在第一步命中项目的 `queryMethods` 中，根据用户的问题或方法名称匹配 `method`。
...
- `references/rds-query-config.json`：项目关键词、RDS 调用参数、业务查询方法、校验规则和 SQL 模板。
