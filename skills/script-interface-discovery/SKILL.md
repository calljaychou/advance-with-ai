...
5. **Explain before calling.** Present a compact numbered list sorted by the stable `key`. For each item show method, path, purpose, required parameters, optional parameters, and an example input shape. Do not call anything during discovery unless the user explicitly requests execution.
...
8. **Resolve a call request.** If the user specifies a number, load the cache entry for the current chat. Confirm the project path and environment match the requested context. If the key is absent or stale, rediscover before calling. If the user specifies a path instead, match it exactly against the current index; do not fuzzy-match silently.
