...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.brainstorm/` to `.gitignore` if it is not already ignored.

**Codex:**
```bash
# Run this with exec_command. If the command keeps running in foreground mode,
# preserve the returned session id and poll it for server output.
scripts/start-server.sh --project-dir /path/to/project --open
```

The server must keep running while the visual companion is in use. If the shell tool returns a session id, keep it until the brainstorming session is finished.
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   ...
   - Use `apply_patch` or another structured edit tool to write files. Avoid shell heredocs.
